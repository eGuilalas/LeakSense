...
// Fetch Gas Detections for GS1 and Populate the Table with Gas Detected Only
function fetchReadings() {
    fetch('../../config/esp_get_readings.php')
        ...
        .then(data => {
            ...
            tableBodyGS1.innerHTML = '';  // Clear the table

            // Loop through each reading and create rows only for GS1 where gas is detected
            data.forEach(reading => {
                if (reading.device_id === 'GS1' && (reading.smoke_status == '1' || reading.co_status == '1' || reading.lpg_status == '1')) {
                    ...
                    `;
                    tableBodyGS1.appendChild(newRow);
                }
            });
